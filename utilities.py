...
def extract_protocol_conf(sequences):
    all_protocol_conf = []
    for sequence in sequences:
        protocol_confs = []
        for feature in sequence:
            protocol_conf = feature[2]
            protocol_confs.append(protocol_conf)
        all_protocol_conf.append(protocol_confs)
    return all_protocol_conf

def extract_src_port(sequences):
    all_ports = []
    for sequence in sequences:
        ports = []
        for feature in sequence:
...
def extract_dst_port(sequences):
    all_ports = []
    for sequence in sequences:
        ports = []
        for feature in sequence:
